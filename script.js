...
recordButton.addEventListener("click", () => {
  // Toggle isRecording and launch necessary function
  isRecording = !isRecording;
  isRecording ? startRecord() : stopRecord();
});

playButton.addEventListener("click", () => {
  // Launch startPlaying only if not recording
  isRecording ? null : startPlaying();
});

// Add click handlers for sound buttons
const soundButtons = document.querySelectorAll(".key");
soundButtons.forEach((button) => {
  button.addEventListener("click", () => {
    const keyCode = parseInt(button.dataset.key);

    // Skip record and play buttons
    if (keyCode !== KEYS.RECORD && keyCode !== KEYS.PLAY) {
      const audioSource = getAudioSource(keyCode);
      playSound(audioSource);

      if (isRecording) {
        recordList.push(keyCode);
        addToLatency();
      }

      playAnimButton(button);
    }
  });
});
...
function handleKeyDown(event) {
  ...
  const divSource = document.querySelector(`.key[data-key="${event.keyCode}"]`);
  ...
  if (
    event.keyCode != recordButton.dataset.key &&
    event.keyCode != playButton.dataset.key
  ) {
